...
void AutomationTestManager::EnqueueTests(const Vector<Name>& testNames)
{
    ...
    {
        if (auto ResultIt = std::find_if(
                std::begin(m_StartupTests), std::end(m_StartupTests),
                [TestName](const SharedPtr<AutomationTest>& It)
                {
                    return It->GetName() == TestName;
                });
            ResultIt != std::end(m_StartupTests))
        {
