...
namespace Zn
{
	...
	NativeThreadHandle WindowsThreads::OpenThread(NativeThreadId thread_id)
	{
		...
		if (Handle == NULL)
		{
			ZN_LOG(LogWindowsThreads, ELogVerbosity::Error, "Unable to open thread with id %d. Error: %d", thread_id, PlatformMisc::GetLastError());
		}
		return Handle;
	}

	NativeThreadHandle WindowsThreads::CreateThread(NativeThreadId& out_thread_id, NativeThreadFunctionPtr function, ThreadArgsPtr args, bool start_suspended /*= false*/)
	{
		DWORD ThreadId;
		NativeThreadHandle Handle = NativeThreadHandle(::CreateThread(NULL, 0, function, args, start_suspended ? CREATE_SUSPENDED : 0, &ThreadId));
