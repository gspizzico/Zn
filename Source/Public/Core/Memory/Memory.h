...
struct MemoryRange
{
  public:
    ...
    constexpr MemoryRange(void* begin, void* end)
        : m_Begin(begin)
        , m_End(end) {};

    MemoryRange(void* begin, size_t size)
        : m_Begin(begin)
        , m_End(Memory::AddOffset(m_Begin, size))
    {
    }

    MemoryRange(const MemoryRange& other, size_t alignment)
        : m_Begin(Memory::Align(other.m_Begin, alignment))
        , m_End(other.m_End)
    {
    }
    ...
    {
        return Memory::GetDistance(address, m_Begin) >= 0 && Memory::GetDistance(address, m_End) < 0;
    }
    ...
    size_t Size() const
    {
        return static_cast<size_t>(Memory::GetDistance(m_End, m_Begin));
    }
