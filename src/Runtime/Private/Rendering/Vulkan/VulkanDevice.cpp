...
{
    Set<u32> queues = {InIndices.Graphics.value(), InIndices.Present.value()};
...
void Zn::VulkanDevice::CreateDescriptors()
{
    ...
    vk::DescriptorSetLayoutCreateInfo globalSetCreateInfo {.bindingCount = SizeOf(bindings), .pBindings = DataPtr(bindings)};
...
void Zn::VulkanDevice::CreateSwapChain()
{
    ...
    if (Indices.Graphics.value() != Indices.Present.value())
    {
        ...
        swapChainCreateInfo.queueFamilyIndexCount = SizeOf(queueFamilyIndicesArray);
        swapChainCreateInfo.pQueueFamilyIndices   = DataPtr(queueFamilyIndicesArray);
    }
...
void Zn::VulkanDevice::CreateDefaultTexture(const String& name, const u8 (&color)[4])
{
    ...
    CopyToGPU(stagingBuffer.allocation, DataPtr(color), SizeOf(color));
...
void Zn::VulkanDevice::CreateMeshPipeline()
{
    ...
    if (vertex_success && fragment_success)
    {
        ...
        vk::DescriptorSetLayoutCreateInfo pbrTexturesSetCreateInfo {
            .bindingCount = SizeOf(pbrBindings),
            .pBindings    = DataPtr(pbrBindings),
        };
        ...
        vk::PipelineLayoutCreateInfo layoutCreateInfo {.setLayoutCount = SizeOf(layouts), .pSetLayouts = DataPtr(layouts)};
